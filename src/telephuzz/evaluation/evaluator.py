...
class DiffEvaluator:
    ...
    def eval(
        ...
        Args:
            ...
            true_request: The Request object as generated by the request generator
        ...
        logger.debug(f"Number of request groups: {len(request_groups)}")
        logger.debug(f"Request groups: {request_groups}")

        if len(request_groups) > 1:
            true_request = max(request_groups, key=lambda r: len(request_groups[r]))
            del request_groups[true_request]
            ...
            for diff_request, libraries in request_groups.items():
                diff_method = true_request.method != diff_request.method
                diff_path = true_request.path != diff_request.path
                diff_parameters = (
                    true_request.query_parameters != diff_request.query_parameters
                )
                diff_body = true_request.body != diff_request.body
                diff_headers = true_request.headers != diff_request.headers
                ...
                for library in libraries:
                    if not any(
                        ...
                        if diff_headers:
                            ...
                            original_content = true_request.headers.get(content_type)
                    ...
                    if diff_method:
                        detail += (
                            f"- Method {true_request.method} expected, "
                            f"but got {diff_request.method}.\n"
                        )
                    if diff_path:
                        detail += (
                            f"- Path {true_request.path} expected, "
                            f"but got {diff_request.path}.\n"
                        )
                    if diff_parameters:
                        detail += (
                            f"- Parameters {true_request.query_parameters} "
                            f"expected, but got {diff_request.query_parameters}.\n"
                        )
                    if diff_body:
                        detail += (
                            f"- Body '{pformat(true_request.body)}' "
